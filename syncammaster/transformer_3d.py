...
class CogVideoXBlock(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self, 'attn_syncam'):
            norm_hidden_states, norm_encoder_hidden_states, gate_msa, enc_gate_msa = self.norm_syncam(
                hidden_states, encoder_hidden_states, temb
            )
            view_num = pose.shape[1]

            # 1. add pose feature
            pose = rearrange(pose, "b v d -> (b v) 1 d")
            pose_embedding = self.cam_encoder(pose)  # (b v) 1 d --> (b v) 1 dim
